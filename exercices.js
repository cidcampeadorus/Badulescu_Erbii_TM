...
function checkAnswer_asymptote(questionNumber) {
  ...
  if (questionNumber === 2) {
    ...
    if (
      userAnswer[0] === answers[questionNumber][0] &&
      userAnswer[1] === answers[questionNumber][1]
    ) {
...
function calculateDerivative() {
  ...
  try {
    ...
    feedback.innerHTML = "Calcul réussi !";
    feedback.style.color = "green";
    ...
    MathJax.typesetPromise([result])
      .then(() => {
        console.log("dérivée calculée");
      })
      .catch((err) => console.log("dérivée non calculée", err));
  } catch (error) {
    feedback.innerHTML = "Erreur dans le calcul. Vérifiez votre entrée.";
    feedback.style.color = "red";
